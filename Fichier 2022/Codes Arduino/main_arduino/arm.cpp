...
// Données globales
float freq = 10; // En Hz, de réception es inputs par le code principal
...
void Arm::angle_processing(){
    ...
    if (inside_c > sqrt(2) / 2 || z < -30) { //empirique : lorsque inside_c = sqrt(2)/2, inside_s aussi et � ces valeurs, arcsin et arccos sont �gaaux : on peut passer d el'un a� l'autre
        ...
        // Probl�me : pour z sufisamment bas, inside_c repasse en dessous de racine2/2, donc on va poser un palier arbitraire dans lequel on utilisera toujours arcsin.
...
float Arm::serial_processing(char* str){
    ...
    // Si la machine est de train de reset, aucune commande ne sera lue tant qu'elle n'a pas fini de le faire.
