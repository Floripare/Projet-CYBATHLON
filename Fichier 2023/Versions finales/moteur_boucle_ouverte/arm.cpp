...
void Arm::angle_processing()
{
    ...
    if (inside_c > sqrt(2) / 2 || z < -30)
    { //empirique : lorsque inside_c = sqrt(2)/2, inside_s aussi et � ces valeurs, arcsin et arccos sont �gaaux : on peut passer d el'un a� l'autre
...
float Arm::serial_processing()
{
    ...
    if (B == 0)
    { // Mode cliqué : X dirige d'avant en arrière.
